import { randomFloat, randomInt, colors } from "./utils.js";
// init canvas
let canvas = document.getElementById('canvas');
canvas.style.background = colors[0];
canvas.setAttribute("width", 64*6);
canvas.setAttribute("height", 64*6);
...
function createMap(numberOfSteps){
    //Create map
...
function draw(arr2d) {
	for(let y = 0; y < arr2d.length; y++){
		for(let x = 0; x < arr2d[0].length; x++){
			ctx.beginPath();
			ctx.fillStyle = arr2d[y][x] ? colors[1] : colors[2];
			ctx.rect(x*tw, y*tw, tw, tw);
			ctx.fill();
		}
	}
}